...
export const card = (() => {
    ...
    const renderAction = (comment) => {
        ...
        if (
            ...
            action += `<button style="font-size: 0.8rem;" onclick="comment.reply(this)" data-uuid="${comment.uuid}" class="btn btn-sm btn-outlined-light rounded-4 py-0 me-1">Reply</button>`;
        }
        ...
        if (
            ...
            action += `<button style="font-size: 0.8rem;" onclick="comment.edit(this)" data-uuid="${comment.uuid}" class="btn btn-sm btn-outlined-light rounded-4 py-0 me-1">Edit</button>`;
        }

        if (session.isAdmin()) {
            action += `<button style="font-size: 0.8rem;" onclick="comment.remove(this)" data-uuid="${comment.uuid}" class="btn btn-sm btn-outlined-light rounded-4 py-0" data-own="${comment.own}">Delete</button>`;
        } else if (
            owns.has(comment.uuid) &&
            (config.get("can_delete") == true ||
                config.get("can_delete") === undefined)
        ) {
            action += `<button style="font-size: 0.8rem;" onclick="comment.remove(this)" data-uuid="${comment.uuid}" class="btn btn-sm btn-outlined-light rounded-4 py-0">Delete</button>`;
        }
    ...
    const renderTracker = (comment) => {
        ...
        <div class="p-2 my-2 rounded-3 border">
            <p class="text-${text} mb-1 mx-0 mt-0 p-0" style="font-size: 0.7rem;" id="ip-${
            comment.uuid
    ...
    const renderHeader = (comment, is_parent) => {
        ...
        return `class="${
            !showHide.get("hidden").find((item) => item.uuid === comment.uuid)[
                "show"
            ]
                ? "d-none"
                : ""
        } card-body overflow-x-scroll mw-100 border-start bg-theme-${btn} py-2 ps-2 pe-0 my-2 ms-2 me-0"`;
    };
    ...
    const renderTitle = (comment, is_parent) => {
        ...
        if (is_parent) {
            return `<strong class="me-1">${util.escapeHtml(
                comment.name
            )}</strong><i id="badge-${comment.uuid}" ></i>`;
        }
    ...
    const renderBody = (comment, is_parent) => {
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center">
            ...
            <small class="text-${text} m-0 p-0" style="font-size: 0.75rem;">${
            comment.created_at
        }</small>
        </div>
        <hr class="text-${text} my-1">
        <p class="text-${text} mt-4 mb-4 mx-0 p-0" style="white-space: pre-wrap !important" id="content-${
            comment.uuid
    ...
    const renderReply = (id) => {
        ...
        <div class="d-flex flex-wrap justify-content-end align-items-center mb-0">
            <button style="font-size: 0.8rem;" onclick="comment.cancel('${id}')" class="btn btn-sm btn-outlined-light rounded-4 py-0 me-1">Cancel</button>
            <button style="font-size: 0.8rem;" onclick="comment.send(this)" data-uuid="${id}" class="btn btn-sm btn-outlined-light rounded-4 py-0">Send</button>
        </div>`;
    ...
    const renderEdit = (id, presence) => {
        ...
        <div class="d-flex flex-wrap justify-content-end align-items-center mb-0">
            <button style="font-size: 0.8rem;" onclick="comment.cancel('${id}')" class="btn btn-sm btn-outlined-light rounded-4 py-0 me-1">Cancel</button>
            <button style="font-size: 0.8rem;" onclick="comment.update(this)" data-uuid="${id}" class="btn btn-sm btn-outlined-light rounded-4 py-0">Update</button>
        </div>`;
