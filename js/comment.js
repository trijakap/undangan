    ...
    const send = async (button) => {
        ...
        const response = await request(HTTP_POST, "/api/comment")
            ...
            .body(
                dto.postCommentRequest(
                    id,
                    nameValue,
                    presence ? presence.value === "1" : true,
                    form.value
                )
            )
        ...
        if (!id) {
            ...
            document.getElementById("comments").innerHTML =
                card.renderContent(response.data) +
                document.getElementById("comments").innerHTML;
            scroll();
        }
        ...
        if (id) {
            ...
            document
                ...
                .insertAdjacentHTML(
                    "beforeend",
                    card.renderInnerContent(response.data)
                );
            ...
            containerDiv
                .querySelector(
                    `button[onclick="like.like(this)"][data-uuid="${id}"]`
                )
                .insertAdjacentHTML(
                    "beforebegin",
                    card.renderReadMore(
    ...
    const comment = () => {
        ...
        return request(
            ...
            .then((res) => {
                ...
                showHide.set(
                    "hidden",
                    traverse(res.data, showHide.get("hidden"))
                );
                return res;
            })
            .then((res) => {
