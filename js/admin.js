...
export const admin = (() => {
        ...
        request(HTTP_GET, "/api/user")
            ...
            .then((res) => {
                ...
                document.getElementById(
                    ...
                    res.data.name
                )}<i class="fa-solid fa-hands text-warning ms-2"></i>`;
                document.getElementById("dashboard-email").innerHTML =
                    res.data.email;
                document.getElementById("dashboard-accesskey").value =
                    res.data.access_key;
                document
                    .getElementById("button-copy-accesskey")
                    .setAttribute("data-copy", res.data.access_key);

                document.getElementById("form-name").value = util.escapeHtml(
                    res.data.name
                );
                document.getElementById("filterBadWord").checked = Boolean(
                    res.data.is_filter
                );
                document.getElementById("replyComment").checked = Boolean(
                    res.data.can_reply
                );
                document.getElementById("editComment").checked = Boolean(
                    res.data.can_edit
                );
                document.getElementById("deleteComment").checked = Boolean(
                    res.data.can_delete
                );
            });
    };
    ...
    const download = async (button) => {
        ...
        await request(HTTP_GET, "/api/download")
            .token(session.getToken())
            .download();
    ...
    const init = () => {
        ...
        if (
            !session.isAdmin() ||
            (JSON.parse(atob((session.getToken() ?? ".").split(".")[1])).exp ??
                0) <
